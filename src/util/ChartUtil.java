...
public class ChartUtil {
    ...
    private static String[] sampleLabels(int length) {
        String[] sampleLabels = new String[length];
    ...
    public static Image getImage(int width, int height,int[] recordList) {
        // 模拟样本数据
        double []sampleValues = sampleValues(recordList);
        // 下方显示的文字
        String[] sampleLabels = sampleLabels(sampleValues.length);
        // 样本中的最大值
    ...
    private static double[] sampleValues(int[] recordList) {
        double[] result = new double[recordList.length];
        for (int i = 0; i < result.length; i++) {
            result[i] = recordList[i];
        }
        return result;
